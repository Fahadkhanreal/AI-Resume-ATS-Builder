...
import { experiencePrompt } from "@/lib/ai/prompts";
import { checkAIRateLimit } from "@/lib/rate-limit";
import { improveExperienceSchema } from "@/lib/schemas/ai";
...
export async function POST(
  ...
  try {
    ...
    const input = improveExperienceSchema.parse(await req.json());
    const result = await generateJsonResponse<{ bullets: string[] }>(
      experiencePrompt(input)
    );
